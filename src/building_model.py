...
class LiteratureReference:
    ...
    reference_story_height_m: float
    reference_plan_width_m: float
    reference_plan_depth_m: float
    reference_gross_density_kg_m3: float
...
REFERENCE_RC_SHEAR_WALL = LiteratureReference(
    ...
    reference_story_height_m=3.5,
    reference_plan_width_m=64.0,
    reference_plan_depth_m=16.0,
    reference_gross_density_kg_m3=300.0,
...
def estimate_target_periods(config: BuildingConfig) -> tuple[float, float]:
    ...
    return _estimate_periods_from_reference_geometry(config, REFERENCE_RC_SHEAR_WALL)
...
def _estimate_periods_from_reference_geometry(
    config: BuildingConfig,
    reference: LiteratureReference,
) -> tuple[float, float]:
    ref_story_mass = (
        reference.reference_gross_density_kg_m3
        * reference.reference_plan_width_m
        * reference.reference_plan_depth_m
        * reference.reference_story_height_m
    )
    current_story_mass = config.floor_mass_kg

    height_ratio = config.total_height_m / reference.reference_height_m
    mass_ratio = current_story_mass / ref_story_mass
    slenderness_ratio = (
        (config.total_height_m / max(config.plan_width_m, 1.0e-9))
        / (reference.reference_height_m / reference.reference_plan_width_m)
    )

    # Keep the estimator coarse-grained: only overall exterior dimensions and
    # lumped mass density enter the period scaling.
    period_scale = (
        height_ratio**1.1
        * mass_ratio**0.35
        * slenderness_ratio**0.15
    )

    return (
        reference.first_mode_period_s * period_scale,
        reference.second_mode_period_s * period_scale,
    )
